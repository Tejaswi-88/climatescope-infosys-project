...
def styled_header(text, level=2, color="rgb(255, 140, 100)", size=30):
    html = f"""
        <h{level} style='
            color: {color};
            font-size: {size}px;
...
if corr_cols:
    ...
    # Dark style setup with smaller size
    ...
    sns.heatmap(
        ...
        ax=ax,
        annot_kws={'color': 'white', 'size': 8}  # smaller annotation font
    )

    # Style adjustments
    ax.set_title(
        "Correlation Between Key Air Quality and Weather Parameters",
