...
def styled_header(text, level=2, color="rgb(255, 140, 100)", size=32):
    html = f"""
        <h{level} style='
            color: {color};
            font-size: {size}px;
...
# 🧭 Temporal Filters
# ============================
styled_header("📆 Temporal Filters")
...
if df_filtered.empty:
    ...
    # Show dataframe in a collapsible expander
    with st.expander("📊 View Filtered Data Sample"):
        if display_columns:
            st.dataframe(
                df_filtered[display_columns].sample(min(100, len(df_filtered))).reset_index(drop=True),
                use_container_width=True
            )
        else:
            st.warning("⚠️ No data columns found in this dataset.")
